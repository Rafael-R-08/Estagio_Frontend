...
import { Eye, EyeOff, Loader2, Mail, Lock, User, BookOpen, Brain } from 'lucide-react';
...
function Badge({ icon: Icon, label }: { icon: React.ElementType; label: string }) {
  return (
    <div className="flex items-center gap-2 rounded-full border border-white/20 bg-white/10 px-4 py-2 text-xs font-medium text-white/90 backdrop-blur-sm transition-all duration-300 hover:scale-105 hover:bg-white/20 cursor-default select-none">
...
export default function RegisterPage() {
  ...
  const onSubmit = async (data: RegisterForm) => {
    try {
      ...
      else setServerError(axiosErr?.message ?? 'Erro ao criar conta.');
    }
  };
  ...
  return (
    <>
      ...
      <style>{`
        ...
        .lh-fade-1     { animation: fadeSlideUp 0.55s 0.08s ease both; }
        .lh-fade-2     { animation: fadeSlideUp 0.55s 0.16s ease both; }
        .lh-fade-3     { animation: fadeSlideUp 0.55s 0.24s ease both; }
        .lh-fade-4     { animation: fadeSlideUp 0.55s 0.32s ease both; }
      ...
      <div className="flex min-h-screen bg-[#F0F4FF]">
        ...
        >
          ...
          <div className="relative z-10 w-full max-w-md px-16 text-center">
            ...
            <p className="lh-fade-2 mb-10 text-base font-light text-blue-200/90 leading-relaxed">
              Junta-te à plataforma de aprendizagem inteligente.<br />
              <span className="font-semibold text-cyan-300">Disponível para todos os colaboradores.</span>
            </p>
        ...
        <div className="flex flex-1 flex-col items-center justify-center px-5 py-10">
          <div className="lh-fade-0 w-full max-w-md">
            ...
            <div className="rounded-3xl bg-white px-10 py-9 shadow-[0_8px_48px_rgba(0,0,0,0.09)] border border-gray-100/80">
              ...
              <div className="mb-7">
                <h2 className="text-2xl font-extrabold text-gray-900 mb-1">Registar conta</h2>
                <p className="text-sm text-gray-400">Regista-te no Softinsa Learning Hub.</p>
              </div>
              ...
              <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
                ...
                <div className="lh-fade-1">
                  <label className="mb-2 block text-sm font-semibold text-gray-700">Nome completo</label>
                  <div className="relative">
                    <User className="absolute left-3.5 top-1/2 h-4 w-4 -translate-y-1/2 text-gray-400 pointer-events-none" />
                    <input
                      type="text"
                      autoComplete="name"
                ...
                {/* Password */}
                <div className="lh-fade-3">
                  <label className="mb-2 block text-sm font-semibold text-gray-700">Palavra-passe</label>
                  <div className="relative">
                    ...
                    >
                      <span className="block transition-transform duration-300" style={{ transform: passwordRotated ? 'rotate(180deg)' : 'rotate(0deg)' }}>
                ...
                {/* Confirm */}
                <div className="lh-fade-4">
                  <label className="mb-2 block text-sm font-semibold text-gray-700">Confirmar palavra-passe</label>
                  <div className="relative">
                    <Lock className="absolute left-3.5 top-1/2 h-4 w-4 -translate-y-1/2 text-gray-400 pointer-events-none" />
                    <input
                      type={showPassword ? 'text' : 'password'}
                      ...
                      className={cn(
                        'lh-input w-full rounded-xl border bg-gray-50 pl-10 pr-4 py-3 text-sm text-gray-900 placeholder:text-gray-400 outline-none transition-all duration-200',
                        errors.confirm ? 'border-red-400 bg-red-50/50' : 'border-gray-200',
                      )}
                      {...register('confirm')}
                    />
                  </div>
